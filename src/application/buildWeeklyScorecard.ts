...
export function buildWeeklyScorecard() {
  ...
  const revenueMap = new Map(computeWeeklyRevenue(repo).map((r: any) => [r.weekStart, r.revenue]));
  const signupsMap = new Map(computeSignups(repo).map((r: any) => [r.weekStart, r.signups]));
  const cancellationMap = new Map(computeCancellationRate(repo).map((r: any) => [r.weekStart, r.cancellationRate]));
  const crewDayMap = new Map(computeRevenuePerCrewDay(repo).map((r: any) => [r.weekStart, r.revenuePerCrewDay]));
  const staffMap = new Map(computeActiveFieldStaff(repo).map((r: any) => [r.weekStart, r.activeStaff]));
